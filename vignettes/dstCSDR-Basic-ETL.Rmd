...
The function names end with the type of file you are importing that CSDR data from, for example:  
...
You then pass the function the full path (including file name and extension) to where the CSDR file you want to import resides on your system, ShareDrive, or network connection.  (The below code won't actually run since this location does not actually exist, but hopefully you get the point.)  

```{r import-direct-path, eval=FALSE, include=TRUE}
# Example of a full function call
...
# Another example of a full function call, this one using `file.choose()`
...
# Import the example files included in the {dstCSDR} package
...
Here is each of those three tables, one at a time (using `gt()` to print them since that looks nicer than `print()` in HTML):  
...
The `{dstCSDR}` package imports the CSDR data so that the initial tables look similar to the original CSDR forms.  In the standard FCHR (DD Form 1921-1), each row of data provides info on a given WBS by functional element code.  The columns in the standard form tell us something about that data by timeframe (i.e., "To Date", "At Completion") and by "Recurring", "Nonrecurring" or "Total".  That format is human readable but is not particularly conducive to exploratory data analysis (EDA) or use in R (or even Excel).  

The `gather_fchr()` and `add_supplemental_fchr_columns()` functions transform the CSDR data from its original format to a tidy/pivot friendly flat file format and combines the reported cost & hours data with reported metadata.  Here is what the data looks like before being transformed:
...
The `gather_fchr()` takes the all the "Cost and Hours..." columns and makes them rows so that each row in the new table gives just one record of cost or hours data (vs. seven columns of data like the default `fchr` data).  Here is what that same data looks like after we run it though `gather_fchr()`:  
...
A close look at the code in the last chunk compared to those that came before revales one notable point:  
...
Now we have the data in a tall (rather than wide) table and it is easier to work with via pivot tables, filters, or other software tools.  Next we can take take our gathered `fchr` data and add metadata from the submission to make it easier to work with multiple CSDR submissions (or CSDRs from multiple program ) at once.  

The `add_supplemental_fchr_columns()` can add any of the metadata as a column along side the gathered `fchr` reported data.

```{r add-supp-fchr}
fchr %>%
  gather_fchr() %>%
  add_supplemental_fchr_columns() %>% 
  .[["reported_data"]] %>%
  head() %>%
  gt() %>%
  tab_options(column_labels.font.size = 11,
